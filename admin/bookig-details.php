...
<head>
	...
	<title>Farm Equipment Hire and Rental Hub | New Bookings   </title>
...
<div id="print">
								<table border="1"  class="display table table-striped table-bordered table-hover" cellspacing="0" width="100%"  >
									...
									<?php 
$bid=intval($_GET['bid']);
									$sql = "SELECT tblusers.*,tblcategory.CategoryName,tblequipment.EqipmentTitle,tblrent.FromDate,tblrent.ToDate,tblrent.message,tblrent.equipmentId as vid,tblrent.Status,tblrent.PostingDate,tblrent.id,tblrent.BookingNumber,
DATEDIFF(tblrent.ToDate,tblrent.FromDate) as totalnodays,tblequipment.PricePerDay
									  from tblrent join tblequipment on tblequipment.id=tblrent.equipmentId join tblusers on tblusers.EmailId=tblrent.userEmail join tblcategory on tblequipment.EquipmentCategory=tblcategory.id where tblrent.id=:bid";
$query = $dbh -> prepare($sql);
