...
$loc=$_POST['loc'];
$sql = "SELECT id from tblequipment where tblequipment.EquipmentCategory=:category and tblequipment.EquipLocation=:loc";
$query = $dbh -> prepare($sql);
...
$sql = "SELECT tblequipment.*, tblcategory.CategoryName, tblcategory.id as bid, tblowner.Name AS owner_name, tblowner.mob AS owner_mob, tblowner.address AS owner_add 
        FROM tblequipment 
        JOIN tblcategory ON tblcategory.id = tblequipment.EquipmentCategory 
        JOIN tblowner ON tblowner.id = tblequipment.ownerid 
        WHERE tblequipment.EquipmentCategory = :category 
        AND tblequipment.EquipLocation = :loc";
